...
describe('ChatView prompt annotation focus and reuse', () => {
  ...
  it('keeps workspace validation and draft hydration cancellation request-local', () => {
    ...
    expect(validation).toContain('timeoutMs: Math.max(1, deadlineAt - Date.now())')
    expect(validation).toContain('signal: controller.signal')
    expect(validation).not.toContain('timeoutAction:')
    expect(validation).not.toContain('abortAction:')
    ...
    expect(draft).toContain('timeoutMs: Math.max(1, deadlineAt - Date.now())')
    expect(draft).toContain('signal: controller.signal')
    expect(draft).not.toContain('timeoutAction:')
    expect(draft).not.toContain('abortAction:')
  })
