...
async function show(event: MouseEvent | KeyboardEvent, file: WorkspaceFile) {
  ...
  invoker = event.target instanceof Element
    ? event.target.closest<HTMLElement>('button, a, [tabindex]')
  ...
  <Teleport to="body">
    <div v-if="visible" class="workspace-file-actions-backdrop" @pointerdown.self="cancel">
      ...
      >
        ...
        <button v-if="nativeOpenAvailable" role="menuitem" type="button" @click="perform('native-open')">
          {{ t('resourceActions.openSource') }}
        </button>
        <button v-if="nativeRevealAvailable" role="menuitem" type="button" @click="perform('reveal')">
...
.workspace-file-actions-menu button { border: 0; border-radius: var(--radius-sm); padding: 8px 12px; background: transparent; color: inherit; text-align: start; font: inherit; font-size: 0.8125rem; cursor: pointer; }
