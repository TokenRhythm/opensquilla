<template>
  <section
    ...
    :data-plan-run-id="kind === 'plan' ? run.runId : undefined"
    role="region"
        ...
        >
          ...
          <Transition name="plan-run-popover">
            ...
            >
              ...
              <div v-if="statusReason || explanation" class="plan-run__popover-reason">
...
const props = withDefaults(defineProps<{
  run: Pick<PlanRunSnapshot, 'runId' | 'status' | 'steps' | 'currentStepId' | 'pauseReason' | 'terminalReason'>
  kind?: 'plan' | 'task'
...
onBeforeUnmount(() => {
  if (typeof document !== 'undefined' && rootElement.value?.contains(document.activeElement)) {
    emit('focusReturn')
  }
})
...
const regionTitle = computed(() => props.kind === 'task'
  ? String(t('chat.goal.progressSummary', {
      completed: completedCount.value,
      total: props.run.steps.length,
    }))
  : String(t('chat.planRun.title')))

const regionLabel = computed(() => {
  if (props.kind === 'task') return `${regionTitle.value}: ${summaryTitle.value}`
...
@media (max-width: 640px) {
  ...
  /* Anchor the wide popover to the centered ribbon, including its Cancel
     control, rather than to the summary button on the left. */
  .plan-run__disclosure {
    position: static;
  }
