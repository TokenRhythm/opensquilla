...
describe('PromptAnnotationAcceptanceQueue', () => {
  ...
  it('merges replayed events that share provisional or canonical identity', async () => {
    const queue = new PageAnnotationSendQueue()
    queue.enqueue({ draftIds: ['annotation-1'], sessionKey: 'canonical' }, 0)
    queue.enqueue({
      draftIds: ['annotation-2'],
  ...
  it('keeps a failed delivery for the next lifecycle pass', async () => {
    const queue = new PageAnnotationSendQueue()
    queue.enqueue({ draftIds: ['annotation-1'], sessionKey: 'session' }, 0)
    let fail = true
  ...
  it('does not delete a replay enqueued while delivery is awaiting the runtime', async () => {
    ...
    queue.enqueue({ draftIds: ['annotation-2'], sessionKey: 'session' }, 2)
    releaseDelivery()
    await firstFlush
    expect(queue.size).toBe(1)
  ...
  it('expires disconnected handoffs and bounds retained entries', async () => {
    const queue = new PageAnnotationSendQueue({ maxEntries: 2, ttlMs: 10 })
    queue.enqueue({ draftIds: ['a'], sessionKey: 'a' }, 0)
    queue.enqueue({ draftIds: ['b'], sessionKey: 'b' }, 1)
    queue.enqueue({ draftIds: ['c'], sessionKey: 'c' }, 2)
