...
async function projected(response: Record<string, unknown>) {
  const call = vi.fn(async () => response) as RpcCall
  return rpcWith(call).usage('all', { timezone: 'UTC' })
}

describe('usage.query compatibility client', () => {
  it('maps UI ranges to calendar-day presets', async () => {
    const call = vi.fn(async () => ({ coverage: { status: 'complete' } })) as RpcCall
    const adapter = rpcWith(call)
    for (const [range, preset] of [
      ['today', 'today'],
      ...
      await adapter.usage(range, { timezone: 'UTC' })
      expect(call).toHaveBeenLastCalledWith(
        'usage.query',
        expect.objectContaining({ range: { preset } }),
        expect.any(Object),
      )
    }
  })
  ...
  it('requests schema v1 with the browser timezone and server dimensions', async () => {
    ...
    const result = await rpc.usage('7', { timezone: 'Asia/Shanghai' })
  ...
  it('uses authoritative server totals instead of summing displayed sessions', async () => {
  ...
  it('normalizes additive native billing fields at every aggregation level', async () => {
    const cny = {
  ...
  it('keeps native billing unavailable for legacy responses', async () => {
    const result = await projected({
      schemaVersion: 1,
      totals: { costNanos: 1 },
      coverage: { status: 'complete' },
  ...
  it('normalizes served fx display rates and drops malformed entries', async () => {
    const result = await projected({
      schemaVersion: 1,
      totals: { costNanos: 1 },
      coverage: { status: 'complete' },
    ...
    const snakeCase = await projected({
      schemaVersion: 1,
      totals: { costNanos: 1 },
      coverage: { status: 'complete' },
  ...
  it('does not turn a deleted session id into a clickable session key', async () => {
    const result = await projected({
      schemaVersion: 1,
      totals: { costNanos: 1 },
      sessions: [{
  ...
  it('treats partial coverage as a successful ledger response', async () => {
    ...
    const result = await rpcWith(call).usage('all', { timezone: 'UTC' })
  ...
  it('does not probe usage.query when Hello does not advertise it', async () => {
    ...
    const result = await rpcWith(call, false).usage('all', { timezone: 'UTC' })
  ...
  it('silently disables a missing method for the connection and falls back', async () => {
    ...
    const result = await rpc.usage('7', { timezone: 'UTC' })
  ...
  it('retries an invalid browser timezone once with UTC', async () => {
    ...
    const result = await rpcWith(call).usage('today', {
  ...
  it('keeps the same-range ledger cache when query and status both fail', async () => {
    ...
    const result = await rpcWith(call).usage('7', {
      timezone: 'Asia/Shanghai',
  ...
  it('does not replace an exact cache with an approximation after a transient query failure', async () => {
    ...
    const result = await rpcWith(call).usage('all', {
      timezone: 'UTC',
      cachedSnapshot: cached,
    })
