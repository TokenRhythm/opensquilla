...
| **On-demand skills and MCP** | 15 bundled skills (coding, GitHub, cron, pptx/docx/xlsx/pdf, summarization, tmux, weather, and more) load only when the task needs them. OpenSquilla includes MCP client and server support; run `opensquilla mcp-server run` to expose its session workflows to another MCP client. Skills can be authored, installed, and published from the CLI. |
