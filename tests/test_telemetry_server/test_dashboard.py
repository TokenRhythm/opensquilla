...
def test_authenticated_page_and_api_contain_aggregates_but_no_telemetry_ids(
    ...
    assert "每日 MetaSkill 使用次数" not in page.text
    assert 'id="metaskill-trend"' not in page.text
    assert "每日编程模式使用次数" not in page.text
    assert "仅开启模式不计数" not in page.text
    assert 'id="coding-mode-trend"' not in page.text
    ...
    assert "metaskillUsage" not in api.json()["growth"]
    assert "codingModeUsage" not in api.json()["growth"]
