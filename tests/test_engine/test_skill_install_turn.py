...
from opensquilla.engine.types import DoneEvent, ToolCall, ToolResultEvent
...
class ScriptedProvider:
    ...
    def __init__(self, turns: list[str | list[tuple[str, dict[str, Any]]]]) -> None:
        self.turns = turns
        self.calls = 0
    ...
    async def chat(self, messages: Any, **kwargs: Any) -> AsyncIterator[Any]:
        ...
        if index >= len(self.turns):
            ...
            return
        if isinstance(self.turns[index], str):
            yield ProviderText(text=self.turns[index])
            yield ProviderDone(stop_reason="end_turn", input_tokens=1, output_tokens=1)
            return
...
class SteeringProvider(ScriptedProvider):
    def __init__(self, turns, pending, *, steer_after: int, steer: str) -> None:
    ...
    async def chat(self, messages: Any, **kwargs: Any) -> AsyncIterator[Any]:
        async for event in super().chat(messages, **kwargs):
            if isinstance(event, ProviderDone) and self.calls == self.steer_after:
                self.pending.append(self.steer)
            yield event
...
@pytest.mark.parametrize(("axis", "state", "hint"), [
    ("selection_state", "disabled", "Enable the Skill before using it."),
    ("selection_state", "shadowed", "Select the intended Skill source."),
    ("readiness_state", "needs_setup", "Install the declared dependencies."),
])
def test_final_receipt_never_claims_unusable_skill_is_ready(axis, state, hint) -> None:
    turn = SkillInstallTurn("安装 demo")
    turn.record("demo", "clawhub", {
        "success": True, "name": "demo", "instruction_usable": False,
        "lifecycle": {axis: state},
        "diagnostics": [{"hint": hint}],
    })
    assert turn.complete
    assert "尚不可用" in turn.final_text()
    assert state in turn.final_text()
    assert hint in turn.final_text()
...
@pytest.mark.asyncio
async def test_plain_response_steer_keeps_followup_work_after_install(setup):
    source, loader, ctx, calls, make_agent = setup
    pending = ListPendingInputProvider()
    provider = SteeringProvider([
        "I can install demo.",
        [("skill_install_community", {"identifier": "demo"})],
        "The requested explanation is complete.",
    ], pending, steer_after=1, steer="After installing, explain the result.")
    events = [event async for event in make_agent(provider).run_turn(
        "install demo", pending_input_provider=pending,
    )]
    assert source.calls == 1
    assert len(pending.applications) == 1
    assert provider.calls == 3
    assert "explanation is complete" in next(e.text for e in events if isinstance(e, DoneEvent))


@pytest.mark.asyncio
async def test_user_directory_choice_releases_failure_but_retains_success(setup, monkeypatch):
    source, loader, ctx, calls, make_agent = setup
    pending = ListPendingInputProvider()
    resolve = source.resolve

    async def resolve_with_choices(identifier):
        if identifier == "collection":
            raise SkillSourceFetchError.diagnostic(
                "SOURCE_TREE_AMBIGUOUS", "Choose a Skill directory",
                phase=DiagnosticPhase.SOURCE,
                details={"candidates": [{"identifier": "bravo"}]},
            )
        return await resolve(identifier)

    monkeypatch.setattr(source, "resolve", resolve_with_choices)
    provider = SteeringProvider([
        [("skill_install_community", {"identifier": "demo"}),
         ("skill_install_community", {"identifier": "collection"})],
        [("skill_install_community", {"identifier": "bravo"})],
        "Choose a directory to install.",
        [("skill_install_community", {"identifier": "demo"}),
         ("skill_install_community", {"identifier": "bravo"})],
    ], pending, steer_after=3, steer="Choose bravo from that collection.")
    events = [event async for event in make_agent(provider).run_turn(
        "install demo and collection", pending_input_provider=pending,
    )]
    assert len(pending.applications) == 1
    assert source.calls == 2
    assert (loader.managed_dir / "demo").is_dir()
    assert (loader.managed_dir / "bravo").is_dir()
    assert ctx.skill_install_turn.previous("bravo", "clawhub")["success"] is True
    premature_choice = next(e for e in events if isinstance(e, ToolResultEvent)
                            and e.tool_use_id == "call-1-0")
    assert json.loads(premature_choice.result)["success"] is False


def test_final_receipt_preserves_degraded_compatibility_and_action_hint():
    turn = SkillInstallTurn("install demo")
    turn.record("demo", "clawhub", {
        "success": True, "name": "demo", "instruction_usable": True,
        "lifecycle": {"compatibility_state": "degraded"},
        "diagnostics": [{"code": "TOOL_PREAPPROVAL_IGNORED",
                         "hint": "Approve individual tool calls when requested."}],
    })
    assert "limited compatibility" in turn.final_text()
    assert "Approve individual tool calls" in turn.final_text()


@pytest.mark.asyncio
@pytest.mark.parametrize("fail_second", [False, True])
async def test_same_batch_installs_settle_before_finalization(setup, monkeypatch, fail_second):
    source, loader, ctx, calls, make_agent = setup
    fetch = source.fetch_resolved

    async def fetch_maybe_failing(resolution):
        if fail_second and resolution.requested_identifier == "bravo":
            raise SkillSourceFetchError.diagnostic(
                "FETCH_FAILED", "Synthetic download failure", phase=DiagnosticPhase.FETCH,
            )
        return await fetch(resolution)

    monkeypatch.setattr(source, "fetch_resolved", fetch_maybe_failing)
    provider = ScriptedProvider([
        [("skill_install_community", {"identifier": "demo"}),
         ("skill_install_community", {"identifier": "bravo"}),
         ("skill_list", {})],
        "The second install failed; the first succeeded.",
    ])
    events = [event async for event in make_agent(provider).run_turn("install demo and bravo")]
    assert (loader.managed_dir / "demo").is_dir()
    assert (loader.managed_dir / "bravo").is_dir() is not fail_second
    assert ctx.skill_install_turn.previous("bravo", "clawhub")["success"] is not fail_second
    assert provider.calls == (2 if fail_second else 1)
    assert calls == ["skill_install_community", "skill_install_community"] + (
        ["skill_list"] if fail_second else []
    )
    done = next(e for e in events if isinstance(e, DoneEvent))
    assert ("second install failed" in done.text) is fail_second


@pytest.mark.asyncio
@pytest.mark.parametrize("arrival", ["during_fetch", "result_delivery"])
async def test_install_completion_preserves_new_user_input(setup, monkeypatch, arrival):
    source, loader, ctx, calls, make_agent = setup
    pending = ListPendingInputProvider()
    steer = "Explain the installed Skill before finishing."
    fetch = source.fetch_resolved

    async def fetch_with_steer(resolution):
        pending.append(steer)
        return await fetch(resolution)

    if arrival == "during_fetch":
        monkeypatch.setattr(source, "fetch_resolved", fetch_with_steer)
    provider = ScriptedProvider([
        [("skill_install_community", {"identifier": "demo"})],
        [("skill_install_community", {"identifier": "demo"})],
        "The requested explanation is complete.",
    ...
    events = []
    async for event in make_agent(provider).run_turn(
        "install demo", pending_input_provider=pending,
    ):
        events.append(event)
        if (arrival == "result_delivery" and isinstance(event, ToolResultEvent)
                and event.tool_use_id == "call-0-0"):
            pending.append(steer)
    assert source.calls == 1
    assert provider.calls == 3
    assert len(pending.applications) == 1
    assert steer in str(provider.requests[1])
    assert "explanation is complete" in next(e.text for e in events if isinstance(e, DoneEvent))


@pytest.mark.asyncio
@pytest.mark.parametrize("valid_confirmation", [False, True])
async def test_user_risk_confirmation_retry_still_validates_artifact(
    setup, monkeypatch, valid_confirmation,
):
    from opensquilla.skills.hub import management

    source, loader, ctx, calls, make_agent = setup
    monkeypatch.setattr(management, "scan_skill_tree", lambda path: ScanResult(verdict="dangerous"))
    pending = ListPendingInputProvider()
    retry = {"identifier": "demo", "force": True}
    provider = ScriptedProvider([
        [("skill_install_community", {"identifier": "demo"})],
        [("skill_install_community", retry)],
    ])
    receipts = []
    async for event in make_agent(provider).run_turn(
        "install demo", pending_input_provider=pending,
        ...
        if not isinstance(event, ToolResultEvent):
            continue
        receipt = json.loads(event.result)
        receipts.append(receipt)
        if event.tool_use_id == "call-0-0":
            diagnostic = next(d for d in receipt["diagnostics"]
                              if d["code"] == "SCAN_CONFIRMATION_REQUIRED")
            token = diagnostic["details"]["confirmationToken"]
            retry["risk_confirmation"] = token if valid_confirmation else "invalid-token"
            pending.append("I approve the reported risk for this artifact; retry installing demo.")
    assert len(pending.applications) == 1
    assert source.calls == 2
    assert receipts[0]["success"] is False
    assert receipts[1]["success"] is valid_confirmation
    assert (loader.managed_dir / "demo").is_dir() is valid_confirmation
