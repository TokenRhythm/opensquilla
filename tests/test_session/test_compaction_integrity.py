...
@pytest.mark.parametrize("failure", ["empty", "length", "error", "oversized"])
async def test_prefix_failure_never_replaces_source(failure):
    class Provider:
...
def test_final_fit_cannot_hide_previously_covered_goals():
    ...
    assert _fit_structured_summary_current_status(summary, max_tokens=80, max_chars=130)
    coverage, reason = validate_compaction_artifact(render_structured_summary(summary), obligations)
    assert reason == "coverage_blocked"
