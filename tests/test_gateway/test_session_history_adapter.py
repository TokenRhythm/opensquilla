...
def test_parse_history_cursor_keeps_null_and_blank_unpositioned() -> None:
...
def test_parse_history_cursor_rejects_nonempty_invalid_values(value: object) -> None:
...
@pytest.mark.asyncio
async def test_adapter_preserves_cursor_invalidation() -> None:
...
@pytest.mark.asyncio
async def test_adapter_does_not_mislabel_a_canonical_read_failure() -> None:
    class BrokenManager(CanonicalManager):
        async def get_canonical_transcript_page(self, *args: Any, **kwargs: Any) -> object:
    ...
    manager = BrokenManager(None)
    adapter = SessionHistoryStorageAdapter(manager)
    with pytest.raises(CanonicalHistoryReadError):
        await adapter.application().read_page(
            SessionHistoryQuery(
                session_key="agent:main:webchat:history",
                limit=1,
