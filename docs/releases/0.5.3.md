...
- Official TokenRhythm HTTPS requests can include the existing pseudonymous
  installation identifier only for the two exact official hosts on port 443.
  It remains optional and untrusted, contains no raw MAC or IP values, and is
  suppressed by the unified privacy control, legacy telemetry opt-out, and
  CI/test detection.
- LAN WebSocket peers are restricted to loopback, private RFC 1918 ranges, or
  IPv6 ULA ranges and can be narrowed further with
  `auth.allowed_client_cidrs`.
- Model discovery adds optional metadata fields. Existing OpenSquilla clients
  may ignore them; external strict decoders that reject unknown fields should
  allow the additions before upgrading.
