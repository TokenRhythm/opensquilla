"""One-way retirement of the discontinued HTML editing protocol's persisted state."""

from __future__ import annotations

# Audit attribution is required: ordinary drafts also have turn IDs and candidates.
LEGACY_CHANGE_SET_QUERY = """
...
RETIREMENT_STATEMENTS: tuple[str, ...] = (
    ...
    WHERE change_set_id IN ({LEGACY_CHANGE_SET_QUERY})
    """,
    f"""
    ...
    WHERE status IN ('reserved', 'ambiguous') AND EXISTS (
        ...
        WHERE change_set.change_set_id IN ({LEGACY_CHANGE_SET_QUERY})
    ...
    WHERE status IN ('draft', 'ready')
      AND change_set_id IN ({LEGACY_CHANGE_SET_QUERY})
    """,
    f"""
    UPDATE artifact_mutation_attempts AS attempt
