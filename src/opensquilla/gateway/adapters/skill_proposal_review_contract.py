...
def _errors(exc: ValidationError) -> tuple[dict[str, Any], ...]:
    return tuple(cast(list[dict[str, Any]], exc.errors(
...
def _binding(method: str) -> GatewayContractBinding[Any]:
    ...
    def validate(result: Any) -> Any:
        try:
            ...
            raise SkillProposalReviewContractError(
    ...
    return GatewayContractBinding(
        ...
        result_validation_errors=(SkillProposalReviewContractError,),
...
_BINDINGS: Final = {
    method: _binding(method) for method in SKILL_PROPOSAL_REVIEW_CONTRACT_METHODS
}
