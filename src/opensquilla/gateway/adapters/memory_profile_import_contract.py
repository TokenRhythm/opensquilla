...
def _result_validator(
    ...
    def validate(result: Any) -> Any:
        try:
            ...
            raise MemoryProfileImportContractError(
...
def _binding(method: str) -> GatewayContractBinding[Any]:
    if method not in MEMORY_PROFILE_IMPORT_CONTRACT_METHODS:
        raise ValueError(f"unsupported profile import Contract method: {method}")
    ...
    return GatewayContractBinding(
        ...
        result_validation_errors=(MemoryProfileImportContractError,),
...
_BINDINGS: Final = {
    method: _binding(method) for method in MEMORY_PROFILE_IMPORT_CONTRACT_METHODS
}
...
def register_memory_profile_import_contract[ContextT, ResultT](
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"unsupported profile import Contract method: {method}") from exc
