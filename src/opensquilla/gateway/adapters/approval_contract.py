...
def _binding(method: str) -> GatewayContractBinding[Any]:
    if method not in APPROVAL_CONTRACT_METHODS:
        raise ValueError(f"unsupported approval Contract method: {method}")
    ...
    return GatewayContractBinding(
        ...
        result_validation_errors=(ApprovalContractError,),
...
_BINDINGS: Final = {method: _binding(method) for method in APPROVAL_CONTRACT_METHODS}
...
def register_approval_contract[ContextT, ResultT](
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"unsupported approval Contract method: {method}") from exc
