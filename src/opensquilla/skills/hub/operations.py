...
class InstallOperationStore:
    ...
    def finish(
        ...
        state: str = "", update_details: bool = False,
    ) -> None:
        if not state:
            state = (
                "succeeded"
    ...
    async def run(
        ...
        if self.store.create(owner, operation_id, signature):
            ...
            async def execute() -> dict[str, Any]:
                ...
                # The worker may add cleanup diagnostics after the durable
                # commit. Refresh its details without changing a proven outcome;
                # restart recovery must not replace them with the older journal.
                self.store.finish(owner, operation_id, result, update_details=True)
                return dict(self.store.read(owner, operation_id)["result"])
